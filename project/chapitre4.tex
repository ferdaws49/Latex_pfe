...
\section{Introduction}
 ...
 Ce chapitre présente de manière structurée le processus de développement adopté pour chaque sprint, en s’appuyant sur le backlog produit et les user stories ayant guidé la conception et l’implémentation des fonctionnalités.
...
Le section \ref{ sec:Sprint1: «Authentification»} consacré à la sécurisation de l’accès à la plateforme via un système d’authentification fiable, tandis que la section \ref{sec:Sprint2: «Gestion des utilisateurs»} traite de la gestion des utilisateurs et des rôles. Ensemble, ces deux sprints constituent une base essentielle pour le développement des fonctionnalités analytiques de la plateforme.
...
\end{tabular}
\end{table}
  Le tableau \ref{tab:sprint1_backlog} présente le backlog du sprint 1, axé sur la fonctionnalité d'authentification, Il détaille les user stories associées, permettant ainsi de planifier et de prioriser les tâches à réaliser pendant ce sprint.
  \subsection{Analyse}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{project/images/connecterdg.png}
    \caption{Diagramme de cas d'utilisation "authentification"}
    \label{fig:usecase_login}
\end{figure}

Le tableau \ref{tab:authentification} présente la description textuelle du cas d’utilisation «Authentification».
\vspace{0.3cm} % petit espace avant le tableau
...
La figure \ref{fig:usecase_password_modification} présente le diagramme de cas d’utilisation expliquant le fonctionnement de la modification du mot de passe.
 \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{project/images/resetDG.png}
    \caption{Diagramme de cas d'utilisation "modification du mot de passe"}
    \label{fig:usecase_password_modification}
\end{figure}
Le tableau \ref{tab:password_modification} présente la description textuelle du cas d’utilisation «modification du mot de passe».
...
\begin{figure}[H]
  \centering
   \includegraphics[scale=0.5]{project/images/inscds.png}
...
\begin{figure}[H]
  \centering
    \includegraphics[scale=0.5]{project/images/Verify-mail.png}
  \caption{Diagramme de séquence <<Vérification de l'adresse email>> }
  \label{cap:Diagramme de séquence <<Vérification de l'adresse email>>}
\end{figure}
...
le diagramme decrire la procédure de connexion sécurisée pour le profil apprenant, qui est schématisé dans la Figure\ref{cap:Diagramme de séquence <<Login - Apprenant>>}.
...
\begin{figure}[H]
  \centering
    \includegraphics[scale=0.8]{project/images/loginds.png}
...
\begin{figure}[H]
  \centering
    \includegraphics[width=1.1\textwidth,keepaspectratio]{project/images/linkressds.png}
...
\begin{figure}[H]
  \centering
    \includegraphics[scale=0.6]{project/images/newpassds.png}
...
\textbf{Interface d'inscription}

La première étape pour l'apprenant est de renseigner ses informations de base via une interface intuitive.Comme l'illustre la Figure \ref{cap:Interface d'inscription:standard}, le formulaire comprend des champs obligatoires avec une aide à la saisie pour guider l'apprenant. La figure \ref{cap:Interface d'inscription:champs-vide} montre comment l'interface réagit lorsque les données ne respectent pas le format attendu (par exemple, un mot de passe trop simple ou un email mal formé). Si un utilisateur tente de s'inscrire avec un email déjà existant, une alerte spécifique est déclenchée comme le montre la figure \ref{cap:Interface d'inscription:champs-vide}.

\textbf{Étape de la Vérification d'email}
...
Dans le cas où l'utilisateur clique sur un lien déjà utilisé ou dont la durée de validité est dépassée, l’interface de la \ref{cap:interface d'inscription:lien non valide} s'affiche pour lui proposer de renvoyer un nouvel email de vérification.
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \label{cap:interfaced'inscription:inscription vérifié}
    \end{minipage}

\end{figure}

\newpage
\textbf{Interface Login}
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \captionof{figure}{Interface de réinitialisation: etat normale}
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\textwidth}
        \centering
...
Enfin, après la validation du nouveau mot de passe, un message de succès s'affiche (voir Figure \ref{cap:reset_success}) et le système redirige automatiquement l'utilisateur vers la page de connexion pour s'authentifier.
...
\label{sec:Sprint2: «Gestion des utilisateurs»}
...
\textbf{Diagramme de séquence: Ajouter un utiliateur}


Le diagramme de séquence de la Figure \ref{cap:Diagramme de séquence <<Ajouter un utilisateur>>} illustre le processus d'ajout d'un nouvel utilisateur par l'administrateur, depuis la saisie du formulaire jusqu'à l'enregistrement en base de données.
\begin{figure}[H]
  \centering
...
La Figure \ref{cap:Diagramme de séquence <<Réinitialisation du mot de passe>>} présente le diagramme de séquence relatif à la réinitialisation du mot de passe d'un utilisateur, initié par l'administrateur.
\begin{figure}[H]
  \centering
