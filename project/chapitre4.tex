...
\end{table}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{project/images/inscritUse.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{project/images/loginUse.png}
...
La figure \ref{fig:usecase_password_modification} présente le diagramme de cas d’utilisation expliquant le fonctionnement de la modification du mot de passe.
 \begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{project/images/resetUsecase.png}
...
le diagramme décrit la procédure de connexion sécurisée pour le profil apprenant, qui est schématisé dans la Figure \ref{cap:Diagramme de séquence <<Login - Apprenant>>}.
...
Enfin, comme le montre la Figure \ref{cap:interface d'inscription:inscription vérifié}, une page de confirmation apparaît brièvement suite à une validation réussie, avant de rediriger automatiquement l'utilisateur vers la page de connexion.
...
\textbf{Interface de création du nouvelle mot de passe :}
...
\begin{center}
    ...
    \begin{minipage}{0.7\textwidth}
        \centering
        \includegraphics[width=0.8\linewidth]{project/images/ress-sucess.png}
...
Le Tableau \ref{tab:backlog_sprint2} détaille le backlog du Sprint 2, centré sur les fonctionnalités de gestion des utilisateurs. Cette structure permet de planifier et de hiérarchiser efficacement les tâches à accomplir tout au long de cette itération.
\begin{table}[h]
\centering
...
La figure \ref{fig:usecase_gestion_utilisateurs} présente le diagramme de cas d'utilisation illustrant les différents scénarios de gestion des utilisateurs, et détaillant les différentes actions que l’administrateur peut réaliser.
 \begin{figure}[H]
    \centering
...
La figure \ref{fig:capture_userstab} présente l'interface principale affichant la liste des utilisateurs sous forme de tableau structuré, avec barre de recherche et filtres par rôle.
...
\textbf{Validation des comptes:} 

La figure \ref{fig:capture_learnersvalidated} illustre l'interface de traitement des demandes d'inscription, permettant à l'administrateur d'accepter ou de refuser les comptes en attente.

\begin{figure}[H]  % [H] bech t7otha houni just
    \centering
    \includegraphics[width=0.8\textwidth]{project/images/localhost_3000_admin_learners.png}
...
La figure \ref{fig:capture_newuser} montre le modal d'ajout d'un nouvel utilisateur, comportant les champs nom, email, rôle et mot de passe.

\begin{figure}[H]  % [H] bech t7otha houni just
    \centering
    \includegraphics[width=0.8\textwidth]{project/images/localhost_3000_admin_users (1).png}
    \caption{Ajout d’un utilisateur}
    \label{fig:capture_newuser}
\end{figure}

\textbf{Modification des informations:} 

 Les figures \ref{fig:interfaces } présentent respectivement la sélection d'un utilisateur et le modal de modification pré-rempli avec ses données actuelles.
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{project/images/localhost_3000_admin_users (2).png}
        \caption{Réinitialisation du mot de passe}
        \label{fig:reset-password}      % <-- Label 1ère image
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{project/images/localhost_3000_admin_users3.png}
        \caption{Modifier d'un utilisateur}
        \label{fig:modifier-user}         % <-- Label 2ème image
    \end{subfigure}
    \caption{Interfaces modification des informations}
    \label{fig:interfaces}               % <-- Label globale (optionnel)
\end{figure}

\textbf{Suppression d’un utilisateur:} 

La figure \ref{fig:capture_del} montre le modal de confirmation de suppression, invitant l'administrateur à confirmer ou annuler la suppression définitive du compte.

\begin{figure}[H]  % [H] bech t7otha houni just
    \centering
    \includegraphics[width=0.8\textwidth]{project/images/localhost_3000_admin_users4.png}
...
L’ensemble du développement a suivi une démarche structurée basée sur l’analyse, la conception et la réalisation. Cette première version constitue une base fonctionnelle importante pour la suite du projet.